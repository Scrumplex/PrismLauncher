...
void ModUpdateDialog::checkCandidates()
{
    ...
    if (!m_failed_metadata.empty()) {
        ...
        auto message_dialog = new ScrollMessageBox(m_parent, tr("Metadata generation failed"),
    ...
    if (!m_failed_check_update.empty()) {
        ...
        auto message_dialog = new ScrollMessageBox(m_parent, tr("Failed to check for updates"),
